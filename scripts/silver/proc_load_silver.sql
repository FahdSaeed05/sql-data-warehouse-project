...
==========================================================================
Script Purpose:
  This stored procedure performs the ETL (Extract< Transfor, Load) process 
...
Parameters:
    None.
    This stored proedure does not accept any values.
...
BEGIN	
	...
	BEGIN TRY
		...
		END cst_marital_status,
			...
			ELSE 'n/a'
			END AS cst_gndr,
		TRY_CONVERT(DATE, cst_create_date, 5) AS cst_create_date
		FROM (
			SELECT
			*,
			ROW_NUMBER() OVER(PARTITION BY cst_id 
		ORDER BY TRY_CONVERT(DATE, cst_create_date, 5) DESC) AS flag_last
